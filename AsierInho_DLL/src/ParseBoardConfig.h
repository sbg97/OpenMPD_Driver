...
class ParseBoardConfig {
  ...
  static bool TransducerPositions(FILE *file, struct BoardConfig &parameters) {
    for (int t = 0; t < parameters.numTransducers; t++)
      if (fscanf(file, "(%f,%f,%f),", &(parameters.positions[3 * t]),
                 &(parameters.positions[3 * t + 1]),
                 &(parameters.positions[3 * t + 2])) == 0)
...
public:
  ...
  static void saveParameters(int boardID, BoardConfig config,
                             const char *path = "./") {
    FILE *file;
    std::stringstream filename;
    ...
    if (!file)
      printf("Could not open configuration file for ID %d.\n Maybe the path "
             "did not exist or the file is already open.\n",
             boardID);
    ...
    for (int t = 0; t < config.numTransducers; t++) {
      fprintf(file, "(%f, %f, %f),", config.positions[3 * t],
              config.positions[3 * t + 1], config.positions[3 * t + 2]);
    }
    fprintf(file, "\n");
