...
void AsierInho_CWrapper_RegisterPrintFuncs(void (*p_Message)(const char *),
...
void AsierInho_CWrapper_readParameters(AsierInho_V2_Handler h,
  ...
  ((AsierInho_V2::AsierInhoBoard_V2 *)h)
      ->readParameters(transducerPositions, transducerNormals, transducerIds,
                       phaseAdjust, amplitudeAdjust, numDiscreteLevels);
}

_AsierInho_Export_V2 size_t
AsierInho_CWrapper_totalTransducers(AsierInho_V2_Handler h) {
  return ((AsierInho_V2::AsierInhoBoard_V2 *)h)->totalTransducers();
}

_AsierInho_Export_V2 size_t
AsierInho_CWrapper_totalBoards(AsierInho_V2_Handler h) {
  return ((AsierInho_V2::AsierInhoBoard_V2 *)h)->totalBoards();
}

_AsierInho_Export_V2 void
AsierInho_CWrapper_numTransducersPerBoard(AsierInho_V2_Handler h,
                                          size_t *out_TransPerBoard) {
  ((AsierInho_V2::AsierInhoBoard_V2 *)h)
      ->numTransducersPerBoard(out_TransPerBoard);
}

void AsierInho_CWrapper_updateMessage(AsierInho_V2_Handler h,
                                      unsigned char *messages) {
  ((AsierInho_V2::AsierInhoBoard_V2 *)h)->updateMessage(messages);
}

void AsierInho_CWrapper_updateMessages(AsierInho_V2_Handler h,
                                       unsigned char *messages,
                                       int numMessages) {
...
bool AsierInho_CWrapper_connectTopBottom(AsierInho_V2_Handler h,
                                         int bottomBoardID, int topBoardID) {
  return (((AsierInho_V2::AsierInhoBoard_V2 *)h)
              ->connect(bottomBoardID, topBoardID));
}

bool AsierInho_CWrapper_connect(AsierInho_V2_Handler h, int numBoards,
                                int *boardIDs, float *matBoardToWorld4x4) {
  return (((AsierInho_V2::AsierInhoBoard_V2 *)h)
              ->connect(numBoards, boardIDs, matBoardToWorld4x4));
}
